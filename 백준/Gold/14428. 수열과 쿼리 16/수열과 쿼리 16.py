...
def search(s,e,cnd):
    ...
    cm = (cs + ce) // 2
    if cs==s and ce==e:
        return [cnd.min_value,cnd.min_index]
    elif cm<s:
        return search(s,e,cnd.right_child)
    elif cm>=e:
        return search(s,e, cnd.left_child)
    else:
        # r1,r2 = search(s,cm,cnd.left_child) , search(cm+1,e,cnd.right_child)
        return min(search(s,cm,cnd.left_child) , search(cm+1,e,cnd.right_child))
...
for _ in range(m):
    ...
    if cmd[0]==2:
        print(search(cmd[1]-1,cmd[2]-1,segtree.root)[1]+1)
    else:
